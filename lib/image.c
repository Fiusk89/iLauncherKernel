...
{
    if (!ilrdfs_read_file(filename))
        return NULL;
    ilfs_node_t *ptr_node = ilrdfs_read_file(filename);
    uint32_t size = ptr_node->size;
    uint8_t *ptr = ilfs_skip_node_header(ptr_node);
    uint32_t i, j, k, x, y, w = (ptr[13] << 8) + ptr[12], h = (ptr[15] << 8) + ptr[14], o = (ptr[11] << 8) + ptr[10];
