...
{
    if (start + sizeof(heap_t) + sizeof(heap_node_t) > end)
        return (void *)NULL;
    if (start + sizeof(heap_t) + sizeof(heap_node_t) > max)
        return (void *)NULL;
    if (end > max)
        return (void *)NULL;
...
static void heap_expand_free_nodes(heap_t *heap)
{
    ...
    while (true)
    {
        ...
        if (heap_node_end)
        {
            heap_node->size = (uint64_t)heap_node_end - (uint64_t)heap_node;
...
void *heap_malloc(heap_t *heap, uint64_t size, uint16_t align)
{
    ...
    {
        uint64_t address = NULL;
        if ((uint64_t)heap_node + full_size + sizeof(heap_node_t) > heap->end)
        {
            heap_expand(heap, ((uint64_t)heap_node + full_size + sizeof(heap_node_t)) - heap->end);
            if ((uint64_t)heap_node + full_size + sizeof(heap_node_t) > heap->end)
                return (void *)NULL;
        }
        heap_node_t *new_heap_node = (heap_node_t *)((uint64_t)heap_node + full_size);
